...
請遵循以下原則：
1. **語言限制**：無論 input language 為何，**必須強制使用「繁體中文 (Traditional Chinese)」** 回答，嚴禁使用英文。
2. **數據導向**：所有論點必須基於提供的 JSON 數據，嚴禁編造數據。
3. **專業語氣**：使用臺灣警政與交通專業術語（如：A1/A2事故、逕行舉發、攔停舉發）。
4. **解決方案**：不僅指出問題，更要根據「3E政策」（工程 Engineering、執法 Enforcement、教育 Education）提出具體建議。
5. **理論基礎**：分析時應隱含運用犯罪學與交通安全理論（如威嚇理論、哈頓矩陣）。
...
請以 Markdown 格式輸出，**務必使用繁體中文撰寫**，直接生成報告內容，無需前言後語。
"""
